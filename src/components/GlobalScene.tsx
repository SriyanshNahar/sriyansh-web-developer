...
const Window=({x,y}:{x:number;y:number})=><group position={[x,y,.03]}><M p={[0,0,0]} s={[1.02,1.82,.08]} c="#16202b" e="#102033"/><M p={[0,0,.06]} s={[.94,1.72,.025]} c="#253d50" e="#17334a"/><M p={[0,0,.085]} s={[.025,1.75,.04]} c="#6b7078"/><M p={[0,.88,.085]} s={[.98,.025,.04]} c="#6b7078"/></group>;
const SideWall=({x,y}:{x:number;y:number})=><group position={[x,y,-.1]}><M p={[0,0,0]} s={[.46,2.72,.72]} c="#171a20"/><M p={[x>0?-0.22:.22,.3,.39]} s={[.03,1.95,.05]} c="#7b6141" e="#3a260f"/></group>;
const WarmStrip=({y}:{y:number})=><group position={[0,y,.18]}><M p={[0,0,0]} s={[8.9,.035,.04]} c="#7b5b33" e="#5a3514"/></group>;
const Sign=({p,children}:{p:V3;children:string})=><group position={p}><M p={[0,0,0]} s={[2.1,.72,.1]} c="#16181d"/><T p={[0,0,.08]} size={.11} color="#d8b98b">{children}</T></group>;
...
const Room=({y,title,sub,kind}:{y:number;title:string;sub:string;kind:'about'|'experience'|'projects'|'graphics'|'skills'|'contact'})=>{
 ...
 return <group position={[0,y,0]}>
   ...
   {[-3.3,-2.2,-1.1,0,1.1,2.2,3.3].map(x=><Window key={x} x={x} y={0}/>)}
   <SideWall x={-4.72} y={0}/><SideWall x={4.72} y={0}/><WarmStrip y={1.16}/><WarmStrip y={-1.16}/>
...
const Studio=()=>{const root=useRef<THREE.Group>(null);const target=useRef(new THREE.Vector3());useFrame(({camera,mouse},d)=>{
 ...
 return <group ref={root}>
   ...
   <M p={[0,-6.95,0]} s={[10.8,.32,3.8]} c="#15171b"/><Sign p={[-5.5,-5.85,.4]}>SN STUDIO</Sign><Sign p={[5.5,-5.85,.4]}>CREATIVE LAB</Sign>{[-4.8,-3.6,3.6,4.8].map(x=><Plant key={x} p={[x,-6.55,.65]}/>)}
